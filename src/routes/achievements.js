...
const ACHIEVEMENTS = [
  ...
  // Wealth
  { id: 'nexals_1000',      title: 'Thousandaire',        desc: 'Accumulate 1,000 nexals total earned', icon: '💰', nexals: 200, target: 1000, field: 'nexals_earned',  category: 'Wealth' },
  { id: 'nexals_5000',      title: 'Five-Figure Club',    desc: 'Earn 5,000 nexals total',            icon: '💵', nexals: 500,  target: 5000, field: 'nexals_earned',   category: 'Wealth' },
];

// Compute current stats for a user
async function getUserStats(userId) {
  const [msgRes, dmRes, friendRes, serverRes, roleRes, chMsgRes, codeRes, decoRes, equippedRes] = await Promise.all([
    pool.query(`SELECT COUNT(*) FROM messages WHERE from_id=$1`, [userId]),
    pool.query(`SELECT COUNT(*) FROM messages WHERE from_id=$1`, [userId]), // same as messages
  ...
  // Count codes redeemed = total decorations owned (each requires a code or purchase)
  const ua = await pool.query(`SELECT achievement_id FROM user_achievements WHERE user_id=$1 AND completed_at IS NOT NULL`, [userId]);
  const completedIds = new Set(ua.rows.map(r => r.achievement_id));

  // Count total nexals earned (sum of claimed achievement rewards)
  const nexalsRes = await pool.query(`SELECT nexals FROM users WHERE id=$1`, [userId]);

  return {
    messages_sent: parseInt(msgRes.rows[0].count),
    dms_sent: parseInt(dmRes.rows[0].count),
    friends_count: parseInt(friendRes.rows[0].count),
    servers_joined: parseInt(serverRes.rows[0].count),
    servers_created: parseInt(roleRes.rows[0].count),
    roles_received: parseInt(serverRes.rows[0].count) > 0 ? parseInt(chMsgRes.rows[0].count) > 0 ? 1 : 0 : 0,
    channel_msgs: parseInt(chMsgRes.rows[0].count),
    codes_redeemed: parseInt(decoRes.rows[0].count),
    decos_equipped: equippedRes.rows[0]?.active_decoration ? 1 : 0,
    decos_owned: parseInt(decoRes.rows[0].count),
    nexals_earned: nexalsRes.rows[0]?.nexals || 0,
    roles_received: parseInt(serverRes.rows[0].count) > 0 ? 1 : 0,
  };
}
...
router.get('/', async (req, res) => {
  ...
  // Get existing user_achievement rows
  const ua = await pool.query(
    'SELECT achievement_id, progress, completed_at, claimed_at FROM user_achievements WHERE user_id=$1',
    [userId]
  );
  const uaMap = {};
  ...
  res.json({ achievements: result, nexals });
});

// Sync progress and auto-complete achievements
