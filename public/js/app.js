...
(function () {
  ...
  window.openCallGame = function(type) { const roomId = activeGameRoomId(); if (!roomId || !socket) return toast('Join a call first', 'error'); socket.emit('call_game_open', { roomId, type }, result => { if (result && result.error) toast(result.error, 'error'); }); };
