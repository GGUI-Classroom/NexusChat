...
(function () {
  ...
  const mentionState = { inputId: null, acId: null, start: -1, items: [] };

  function setupMentionAutocomplete(inputId, acId, context) {
    const input = $(inputId);
    if (!input) return;
    input.addEventListener('input', () => checkMentionTrigger(inputId, acId, context));
    input.addEventListener('keydown', e => {
      if (e.key === 'Escape' && mentionState.inputId === inputId) closeMentionAC();
    });
  }

  setupMentionAutocomplete('channel-message-input', 'channel-mention-autocomplete', 'channel');
  setupMentionAutocomplete('message-input', 'dm-mention-autocomplete', 'dm');

  function checkMentionTrigger(inputId, acId, context) {
    ...
    if (charBefore && !/\s/.test(charBefore)) { closeMentionAC(); return; }
    const query = before.slice(atIdx + 1);
    if (!/^[\w-]{0,32}$/.test(query)) { closeMentionAC(); return; }
    mentionState.inputId = inputId;
    mentionState.acId = acId;
    mentionState.start = atIdx;
    mentionState.items = buildMentionItems(context, query.toLowerCase());
    if (!mentionState.items.length) { closeMentionAC(); return; }
    renderMentionList();
    $(acId).style.display = 'block';
  }

  function buildMentionItems(context, q) {
    if (context === 'dm') {
      const people = [activeDmUser, currentUser].filter(Boolean);
      const seen = new Set();
      return people
        .filter(person => {
          if (!person.id || seen.has(person.id)) return false;
          seen.add(person.id);
          const haystack = `${person.displayName || ''} ${person.username || ''}`.toLowerCase();
          return haystack.includes(q);
        })
        .slice(0, 6)
        .map(person => ({ type: 'user', id: person.id, name: person.displayName || person.username, username: person.username, avatarDataUrl: person.avatarDataUrl }));
    }
    if (!activeServerData) return [];
    const special = [
      { type: 'special', id: 'everyone', name: 'everyone', sub: 'Notify everyone in this server' },
      { type: 'special', id: 'here', name: 'here', sub: 'Notify people currently online here' }
    ].filter(item => item.name.startsWith(q));
    const roles = (activeServerData.roles || [])
      .filter(r => r.name.toLowerCase().includes(q))
      .slice(0, 6)
      .map(r => ({ type: 'role', id: r.id, name: r.name, color: r.color }));
    const members = (activeServerData.members || [])
      .filter(m => `${m.displayName || ''} ${m.username || ''}`.toLowerCase().includes(q))
      .slice(0, 8)
      .map(m => ({ type: 'user', id: m.id, name: m.displayName, username: m.username, avatarDataUrl: m.avatarDataUrl }));
    return [...special, ...roles, ...members].slice(0, 12);
  }

  function renderMentionList() {
    const ac = $(mentionState.acId);
    ac.innerHTML = mentionState.items.map((item, i) => {
      if (item.type === 'special') {
        return `<button type="button" class="mention-item" data-mention-idx="${i}">
          <div class="mention-item-icon mention-item-special-icon">@</div>
          <div><div class="mention-item-name">@${esc(item.name)}</div><div class="mention-item-sub">${esc(item.sub)}</div></div>
        </button>`;
      }
      if (item.type === 'role') {
        return `<button type="button" class="mention-item" data-mention-idx="${i}">
    ...
    ac.querySelectorAll('[data-mention-idx]').forEach(btn => {
      btn.addEventListener('mousedown', e => e.preventDefault());
      btn.addEventListener('click', () => insertMention(mentionState.items[Number(btn.dataset.mentionIdx)]));
    });
  }

  function insertMention(item) {
    const ta = $(mentionState.inputId);
    if (!ta || !item) return;
    const val = ta.value;
  ...
  function closeMentionAC() {
    if (mentionState.acId && $(mentionState.acId)) $(mentionState.acId).style.display = 'none';
    ['channel-mention-autocomplete', 'dm-mention-autocomplete'].forEach(id => { if ($(id)) $(id).style.display = 'none'; });
    mentionState.inputId = null;
    mentionState.acId = null;
    mentionState.start = -1;
    mentionState.items = [];
  }

  document.addEventListener('click', e => {
    const activeAc = mentionState.acId && $(mentionState.acId);
    const activeInput = mentionState.inputId && $(mentionState.inputId);
    if (activeAc && !activeAc.contains(e.target) && e.target !== activeInput) closeMentionAC();
  });
  ...
  async function connectSocket() {
    ...
    socket.on('pro_updated', ({ active, expiresAt }) => {
      if (!currentUser) return;
    ...
    socket.on('mentioned', ({ type, serverId: sid, fromUser, preview, special }) => {
      if (type === 'dm') {
        toast(`DM mention from ${fromUser.displayName}: ${preview}`, 'info', 6000);
        return;
      }
      const serverName = servers.find(s => s.id === sid)?.name || 'a server';
      const label = special ? `@${special}` : '@mention';
  ...
  async function adminLookupUser() {
    ...
    const proText = data.proActive
      ? `Active until ${new Date(data.proExpiresAt * 1000).toLocaleDateString()}`
      : 'Inactive';
    result.innerHTML = `
      <div style="background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius-md);padding:16px;margin-top:4px;display:flex;flex-direction:column;gap:14px">
        ...
        ${isCoreAdmin ? `<!-- Nexus Pro -->
        <div style="background:var(--bg-surface);border-radius:8px;padding:12px">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:8px">
            <div style="font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--text-muted)">Nexus Pro</div>
            <span style="font-size:11px;color:${data.proActive ? 'var(--green)' : 'var(--text-muted)'}">${proText}</span>
          </div>
          <div style="display:grid;grid-template-columns:minmax(110px,1fr) auto auto;gap:8px;align-items:center">
            <select id="admin-pro-days" style="min-height:34px;background:var(--bg-void);border:1px solid var(--border-bright);color:var(--text-primary)">
              <option value="30">30 days</option>
              <option value="90">90 days</option>
              <option value="180">180 days</option>
              <option value="365">365 days</option>
            </select>
            <button class="btn-primary" style="font-size:11px;padding:7px 12px" onclick="adminGivePro('${data.id}')">${data.proActive ? 'Extend Pro' : 'Give Pro'}</button>
            <button class="btn-secondary" style="font-size:11px;padding:7px 12px" onclick="adminRemovePro('${data.id}')" ${data.proActive ? '' : 'disabled'}>Revoke</button>
          </div>
        </div>` : ''}
        ...
        ${isCoreAdmin ? `<!-- Nameplates -->
        <div style="background:var(--bg-surface);border-radius:8px;padding:12px">
          <div style="font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--text-muted);margin-bottom:8px">Nameplates</div>
          <div style="display:flex;flex-direction:column;gap:4px;max-height:210px;overflow-y:auto">
            ${(data.nameplates||[]).map(nameplate=>`
              <div style="display:flex;align-items:center;gap:8px;padding:5px 6px;border-radius:6px;background:var(--bg-hover)">
                <span style="font-size:11px;font-weight:700;color:${rarityColor[nameplate.rarity]||'#fff'};flex:1">${esc(nameplate.name)}${nameplate.active ? ' <span style="color:var(--green)">• active</span>' : ''}</span>
                <span style="font-size:10px;color:var(--text-muted)">${nameplate.rarity}</span>
                ${nameplate.owned
                  ? `<button class="action-btn danger" style="font-size:10px;padding:3px 8px" onclick="adminRemoveNameplate('${data.id}','${nameplate.id}')">Remove</button>`
                  : `<button class="action-btn ghost" style="font-size:10px;padding:3px 8px" onclick="adminGiveNameplate('${data.id}','${nameplate.id}')">Give</button>`}
              </div>`).join('')}
          </div>
        </div>` : ''}
  ...
  window.adminGiveNameplate = async function(userId, nameplateId) {
    const r = await api('POST', '/api/admin/users/' + userId + '/nameplates', { nameplateId });
    if (r.error) return toast(r.error, 'error');
    toast('Nameplate granted', 'success');
    await adminLookupUser();
  };

  window.adminRemoveNameplate = async function(userId, nameplateId) {
    if (!confirm('Remove this nameplate from the user?')) return;
    const r = await api('DELETE', '/api/admin/users/' + userId + '/nameplates/' + nameplateId);
    if (r.error) return toast(r.error, 'error');
    toast('Nameplate removed', 'info');
    await adminLookupUser();
  };

  window.adminGivePro = async function(userId) {
    const days = parseInt($('admin-pro-days')?.value, 10) || 30;
    const r = await api('POST', '/api/admin/users/' + userId + '/pro', { days });
    if (r.error) return toast(r.error, 'error');
    toast(`Nexus Pro extended by ${r.days} days`, 'success');
    await adminLookupUser();
  };

  window.adminRemovePro = async function(userId) {
    if (!confirm('Revoke Nexus Pro from this user?')) return;
    const r = await api('DELETE', '/api/admin/users/' + userId + '/pro');
    if (r.error) return toast(r.error, 'error');
    toast('Nexus Pro revoked', 'info');
    await adminLookupUser();
  };
  ...
  function renderBlackjackSetup(table = null) {
    const target = $('games-blackjack-table');
    if (!target) return;
    const existingTokens = table && table.chips > 0;
    const maxBet = existingTokens ? table.chips : nexalsToTableTokens(900);
    target.innerHTML = `<div class="blackjack-buyin-panel blackjack-setup-panel"><div><b>${existingTokens ? 'Next Blackjack Hand' : 'Blackjack Buy-In'}</b><span>${existingTokens ? `${table.chips.toLocaleString()} table tokens available. Choose the next hand bet.` : 'Choose how many Nexals become table tokens before the first hand.'}</span></div>${existingTokens ? '' : '<label>Nexal buy-in <input id="blackjack-buyin-input" type="number" min="1" max="100000" step="450" value="900"></label>'}<label>Token bet <input id="blackjack-bet-input" type="number" min="1" max="${maxBet}" step="50" value="${Math.min(100, maxBet)}"></label><span id="blackjack-token-preview">${existingTokens ? 'Closing Nexus automatically cashes out and resets this table.' : '900 Nexals = 1,000 table tokens. Closing Nexus automatically cashes out and resets this table.'}</span><button class="btn-primary" onclick="openStatsBlackjack()">${existingTokens ? 'Deal Next Hand' : 'Buy In And Deal'}</button></div>`;
    $('blackjack-buyin-input')?.addEventListener('input', updateBlackjackTokenPreview);
  }
  function renderStatsBlackjack(table) {
    lastStatsBlackjackTable = table;
    const target = $('games-blackjack-table');
    if (!target) return;
    const canCashout = table.handsSinceCashout >= 15 && table.chips >= 1000;
    target.innerHTML = `<div class="blackjack-window" id="blackjack-window"><div class="blackjack-window-bar" id="blackjack-window-bar"><div><span class="blackjack-table-mark">N</span><b>Blackjack Table</b><small>vs Nexus Dealer</small></div><div><button class="blackjack-window-btn" id="blackjack-minimize" title="Minimize">_</button></div></div><div class="blackjack-window-body"><div class="blackjack-table-felt"><div class="blackjack-head"><b>Dealer${table.dealerScore !== null ? ' - ' + table.dealerScore : ''}</b><span>${(table.chips || 0).toLocaleString()} tokens | Bet ${(table.bet || 0).toLocaleString()}${table.buyIn ? ' | ' + table.buyIn.toLocaleString() + ' Nexal buy-in' : ''}</span></div>${blackjackCards(table.dealerHand)}<div class="blackjack-seat"><span>YOUR HAND</span><b>${table.playerScore}</b></div>${blackjackCards(table.playerHand)}${table.result ? `<p class="blackjack-result">${esc(table.result)}</p>` : ''}<div class="blackjack-cashout"><span>${table.handsSinceCashout}/15 hands until cashout</span><button class="btn-secondary" onclick="cashoutBlackjack()" ${canCashout ? '' : 'disabled'}>Cash Out ${Math.floor(table.chips / 1000) * 900} Nexals</button></div><div class="game-actions">${table.status === 'playing' ? '<button class="btn-secondary" onclick="statsBlackjackAction(\'hit\')">Hit</button><button class="btn-primary" onclick="statsBlackjackAction(\'stand\')">Stand</button>' : '<button class="btn-primary" onclick="renderBlackjackSetupFromTable()">New Hand</button>'}<button class="btn-secondary" onclick="closeStatsBlackjack()">Leave Table</button></div></div></div></div>`;
  ...
  async function loadGamesHub() {
    $('games-content').innerHTML = `<div class="games-grid"><button class="game-hub-card blackjack" onclick="renderBlackjackSetup()"><span class="game-hub-icon">21</span><strong>Blackjack</strong><small>Beat the dealer</small><p>Open a table, choose table tokens, then set your hand bet.</p><b>Play Solo</b></button><button class="game-hub-card poker" onclick="openCallGamesFromHub()"><span class="game-hub-icon">&#9824;</span><strong>Texas Hold'em</strong><small>Call table</small><p>Private cards, community cards, and a shared pot with up to six players.</p><b>Play In A Call</b></button></div><div id="games-blackjack-table"></div>`;
  }
